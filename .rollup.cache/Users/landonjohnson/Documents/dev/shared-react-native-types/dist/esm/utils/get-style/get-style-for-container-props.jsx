export const getStyleForContainerProps = ({ maxWidth, maxHeight, height, theme, marginTop, width, backgroundColorFromTheme, marginBottom, minHeight, opacity, marginLeft, marginRight, paddingLeft, borderColorFromTheme, paddingRight, margin, borderWidth, flex, paddingTop, }) => {
    const handleUnitProps = theme.handleUnitProps;
    const handleColorFromTheme = theme.handleColorFromTheme;
    const css = `${backgroundColorFromTheme
        ? `background: ${handleColorFromTheme(backgroundColorFromTheme, 1, theme)};`
        : ''}
  ${flex ? `flex: ${flex};` : ''}
...
//# sourceMappingURL=get-style-for-container-props.jsx.map