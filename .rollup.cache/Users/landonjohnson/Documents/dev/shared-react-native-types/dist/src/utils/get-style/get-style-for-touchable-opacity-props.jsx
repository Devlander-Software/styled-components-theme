export const getStyleForTouchableOpacityProps = ({ borderBottomWidth, maxWidth, borderRadius, paddingBottom, alignItems, justifyContent, padding, theme, marginTop, width, backgroundColorFromTheme, marginBottom, marginLeft, marginRight, paddingLeft, paddingRight, flex, paddingTop, alignSelf, }) => {
    const css = [];
    if (backgroundColorFromTheme) {
        css.push(`background-color: ${theme.handleColorFromTheme(backgroundColorFromTheme, 1, theme)};`);
    }
    if (flex) {
        css.push(`flex: ${flex};`);
    }
    ...
    if (borderBottomWidth) {
        css.push(`border-bottom-width: ${theme.handleUnitProps(borderBottomWidth)};`);
    }
    return css.join(' ');
};
//# sourceMappingURL=get-style-for-touchable-opacity-props.jsx.map